...
// Rule
// - cache can send one request to the IU per cycle
...
address_t cache_t::gen_address(address_tag_t tag, int set, int offset) {
...
cache_access_response_t cache_t::lru_replacement(address_t addr) {
  ...
  for (int a = 0; a < assoc; ++a) {
    if (tags[set][a].replacement == -1) {
      car.way = a;
      car.permit_tag = tags[set][a].permit_tag;
      car.address_tag = tags[set][a].address_tag;
      return(car);
  ...
  if (cache_access(addr, SHARED, &car)) {
    ...
    *data = read_data(addr, car);
    ...
    if (!retried_p) {
      ...
      cache_access_response_t car = lru_replacement(addr);
      if (car.permit_tag != INVALID) {
        send_writeback_req = true;
        send_read_req = false;
      } else {
        send_writeback_req = false;
        send_read_req = true;
    ...
    if (send_read_req) {
      ...
      proc_cmd_t proc_cmd = (proc_cmd_t){READ, addr, tag, EXCLUSIVE};
      ...
      // this might fail if we're waiting for writeback to be done, but
      // it's fine since we retry until a cache hit
    ...
    if (send_writeback_req) {
      cache_access_response_t car = lru_replacement(addr);
      address_t replace_address = gen_address( car.address_tag, car.set, 0);
      proc_cmd_t proc_cmd;;
      if (car.permit_tag == MODIFIED) {
        proc_cmd = (proc_cmd_t){WRITE,      replace_address, tag, EXCLUSIVE};
        copy_cache_line( proc_cmd.data , tags[car.set][car.way].data);
      } else {
        proc_cmd = (proc_cmd_t){INVALIDATE, replace_address, tag, EXCLUSIVE};
      }

      //NOTE_ARGS(("%d: miss: addr %d, tag %d", node, addr, tag));

      // issue an invalidate to next level of memory hierarchy.  In this case,
      // it's the IU
      
      iu->from_proc(proc_cmd);

      // before sending the read command, cache needs to wait until
...
response_t cache_t::store(address_t addr, bus_tag_t tag, int data, bool retried_p) {
  ...
  case INVALID: {
    ...
    if (send_read_req) {
      ...
      // this might fail if we're waiting for writeback to be done, but
      // it's fine since we retry until a cache hit
      bool temp = iu->from_proc(proc_cmd);
      // r.retry_p = iu->from_proc(proc_cmd);
    }
...
void cache_t::reply(proc_cmd_t proc_cmd) {
  ...
  if(send_read_req) {
    ...
    car.address_tag = gen_address_tag(proc_cmd.addr);
